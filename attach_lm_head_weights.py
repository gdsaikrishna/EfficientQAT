...
scales = torch.load('scales.pt', weights_only=True)
qweight = torch.load('qweight.pt', weights_only=True)
qzeros = torch.load('qzeros.pt', weights_only=True)
g_idx = torch.load('g_idx.pt', weights_only=True)

w_bits = 4
dq_weights = dequantization(qweight, qzeros, scales, g_idx,w_bits, 64)
model.lm_head.weight = torch.nn.Parameter(dq_weights.detach().cpu().contiguous())
